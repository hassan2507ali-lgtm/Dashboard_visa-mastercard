...
export default function AcquiringDetail({ setCurrentPage }) {
  const [chartData, setChartData] = useState(initialAcquiringDetail);
  const [donutData, setDonutData] = useState(initialDonutAcquiring);
  ...
  const handleChartSubmit = () => {
    ...
    setChartData(chartData.map(item => ({ ...item, volume: +(Math.random() * 8 + 3).toFixed(1), serviceFee: +(Math.random() * 40 + 90).toFixed(1), costToVol: +(Math.random() * 0.8 + 1.2).toFixed(1), interchange: +(Math.random() * 0.8 + 1.2).toFixed(1) })));
  ...
  const getSidebarItemStyle = (pageName) => pageName === 'acquiring-detail' ? "p-2.5 bg-slate-100 text-slate-800 rounded-xl cursor-pointer transition-all shadow-sm ring-1 ring-slate-200" : "p-2.5 text-gray-400 hover:text-gray-700 hover:bg-gray-50 rounded-xl cursor-pointer transition-all";
        ...
        <div className="flex justify-between items-start mb-8">
          <div>
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Detail Transaksi - ACQUIRING</h1>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
          <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col">
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center gap-3"><ShoppingCart className="w-6 h-6 text-gray-700" strokeWidth={2} /><h3 className="font-bold text-xl italic tracking-tight text-gray-900">ACQUIRING</h3></div>
              <span className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide">Trend Analysis Q1 - Q6</span>
            </div>
            <div className="flex justify-around items-end mb-6 px-1 min-h-[40px]"><ChartLegend color="#3b82f6" label="Service fee" /><ChartLegend color="#eab308" label="Interchange" /><ChartLegend color="#cbd5e1" label="Cost To Vol" /><ChartLegend color="#3b82f6" label="Volume" isBar={true} /></div>
            <div className="h-64 w-full text-xs font-semibold mt-2">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={chartData} margin={{ top: 25, right: 15, left: 15, bottom: 0 }}>
                  ...
                  <Line yAxisId="int" type="linear" dataKey="interchange" name="Interchange" stroke="#eab308" strokeWidth={2.5} dot={{ r: 4.5, fill: '#ffffff', stroke: '#eab308', strokeWidth: 2 }} animationDuration={1000}><LabelList dataKey="interchange" position="top" fill="#a16207" formatter={(val) => `${val}%`} offset={10} fontSize={11} /></Line>
          ...
          <div className="lg:col-span-1 bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col overflow-hidden">
            <div className="bg-[#2e5ea8] text-white text-center py-4 text-base font-bold tracking-wide shadow-inner">ACQUIRING 2026</div>
